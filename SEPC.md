...
创建这些文件：
1. `package.json`（Monorepo根）
2. `turbo.json`（Turbo配置）
3. `prisma/schema.prisma`（数据库模型）
4. `packages/server/src/index.ts`（服务端入口）
5. `apps/web/src/main.tsx`（Web端入口）
